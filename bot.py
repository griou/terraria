...
TERRARIA_COMMAND_EXIT = (
    'echo exit | socat EXEC:"docker attach %s",pty STDIN' % CONTAINER_NAME
)
...
TERRARIA_COMMAND_IS_RUNNING = (
    "docker inspect --format '{{json .State.Running}}' %s" % CONTAINER_NAME
)
...
async def on_raw_reaction_add(payload: discord.RawReactionActionEvent):
    ...
    if terraria_is_running() and file_exists:
        ...
        await channel.send(
            "Terraria server is running and current server is using same filename. Stop server before replacing the file `%s`"
            % filename
        )
        return
    ...
    await channel.send(
        "Terraria map `%s` uploaded to server :white_check_mark:" % filename
    )
...
async def bot_terraria_backup(ctx, filename):
    ...
    await discord_reaction_done(ctx.message)
    await ctx.send(
        file=file, content="Terraria map `%s` backup :page_facing_up:" % filename
    )
...
async def bot_terraria_update(ctx):
    ...
    if updated:
        await ctx.send(
            content="Terraria image server version updated :gear:. Restart server to complete update"
        )
    else:
...
async def discord_reaction(message, emoji, add=True):
    await message.add_reaction(emoji) if add else await message.remove_reaction(
        emoji, bot.user
    )
...
def terraria_start(filename):
    os.system(
        TERRARIA_COMMAND_START % (CONTAINER_NAME, TERRARIA_FOLDER, filename, IMAGE_NAME)
    )
...
def terraria_update():
    ...
    previous_creation_date = (
        client.images.get("beardedio/terraria").history()[0].get("Created")
    )
